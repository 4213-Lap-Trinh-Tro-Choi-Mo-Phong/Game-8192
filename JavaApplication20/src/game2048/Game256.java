...
public class Game256 extends JPanel 
{
  private static final Color BG_COLOR = new Color(0xa9a9a9);
  private static final String FONT_NAME = "Arial";
  private static final int TILE_SIZE = 32;
  private static final int TILES_MARGIN = 8;
...
// Di chuyển trái
  ...
  boolean canMove() {
    ...
    for (int x = 0; x < 16; x++) 
    {
      ...
      {
        Tile t = tileAt(x, y);
        if ((x < 15 && t.value == tileAt(x + 1, y).value)
          || ((y < 15) && t.value == tileAt(x, y + 1).value)) 
        {
          return true;
        }
      }
  ...
  private Tile[] rotate(int angle) 
  {
    ...
    for (int x = 0; x < 16; x++) 
    {
      for (int y = 0; y < 16; y++) 
      {
        ...
        newTiles[(newX) + (newY) * 16] = tileAt(x, y);
      }
    }
    return newTiles;
  }
  ...
  private Tile[] mergeLine(Tile[] oldLine) 
  {
    ...
    for (int i = 0; i < 16 && !oldLine[i].isEmpty(); i++) 
    {
      int num = oldLine[i].value;
      if (i < 15 && oldLine[i].value == oldLine[i + 1].value) 
      {
        num *= 2;
        myScore += num;
        int ourTarget = 64;
    ...
    if (list.size() == 0) 
    {
      ...
      ensureSize(list, 16);
      return list.toArray(new Tile[16]);
    }
  }
  ...
  {
    System.arraycopy(re, 0, myTiles, index * 16, 16);
  }
 ...
 //Giao diện
  ...
  private void drawTile(Graphics g2, Tile tile, int x, int y) 
  {
    ...
    final int size = value < 2000 ? 36 : value < 3000 ? 32 : 24;
    ...
    if (value != 0)
      g.drawString(s, xOffset + (TILE_SIZE - w) / 2, yOffset + TILE_SIZE - (TILE_SIZE - h) / 2 - 2);
        ...
        if (myWin || myLose) 
        {
          ...
          if (myWin) 
          {
            g.drawString("Chiến thắng!", 200, 300);   
          }
          if (myLose) 
          {        
            g.drawString("Bạn thua!", 200, 300);
          }
          if (myWin || myLose) 
          {
            g.setFont(new Font(FONT_NAME, Font.PLAIN, 16));
            g.setColor(new Color(128, 128, 128, 128));
            g.drawString("Nhấn ESC để chơi lại", 60, getHeight() - 40);
          }
        }
    g.setFont(new Font(FONT_NAME, Font.PLAIN, 20));
    g.drawString("Điểm: " + myScore, 500, 690);
  }
